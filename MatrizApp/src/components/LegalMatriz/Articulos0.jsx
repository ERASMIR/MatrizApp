...
const Articulos = ({ data = [], onSelectionChange, setSeleccionesParent, seleccionMasiva, setSeleccionesAplicaArticulos }) => {
  ...
  // useEffect(() => {
  //   setSeleccionesParent(selecciones);  // Sincroniza las selecciones con el padre
  // }, [selecciones, setSeleccionesParent]);  // Cada vez que `selecciones` cambia, se actualiza en el componente padre
  ...
  // useEffect(() => {
  //   if (seleccionMasiva) {
  //     setSelecciones(seleccionMasiva);
  //   }
  // }, [seleccionMasiva]);
    ...
    const fetchValoresAplica = async () => {
      if (empresaSeleccionada && locacionSeleccionada && filteredData.length > 0) {
        try {
          ...
          //   const selectAplicaValor = valorAplica?.select_aplica !== null && valorAplica?.select_aplica !== undefined
          //     ? valorAplica?.select_aplica
          //     : 0; // Valor por defecto si es null
  
          //   nuevasSelecciones[articuloId] = {
          //     ...valorAplica,
          //     selectAplica: selectAplicaValor
          //   };
  
          //   // Sincroniza el selectedOption para que los checkboxes reflejen el valor correcto
          //   nuevoSelectedOption[articuloId] = selectAplicaValor;
          // }
  ...
  // Manejar selección masiva desde el padre
  // useEffect(() => {
  //   if (seleccionMasiva) {
  //     const nuevasSelecciones = {};
  //     const nuevoSelectedOption = {};

  //     filteredData.forEach(item => {
  //       nuevasSelecciones[item.id] = {
  //         articuloId: item.id,
  //         empresaId: empresaSeleccionada.id,
  //         locacionId: locacionSeleccionada.id,
  //         fechaRegistro: new Date().toISOString().split('T')[0],
  //         selectAplica: seleccionMasiva
  //       };
  //       nuevoSelectedOption[item.id] = seleccionMasiva;
  //     });

  //     setSelecciones(nuevasSelecciones);
  //     setSelectedOption(nuevoSelectedOption);
  //   }
  // }, [seleccionMasiva, filteredData]);
  ...
  const handleCheckboxChange = (itemId, optionValue) => {
    ...
    setSelecciones(prevState => {
      ...
      //const filteredSelections = Object.fromEntries(
        //Object.entries(updatedSelections).filter(([_, value]) => value !== undefined)
      //);

      //console.log("Selecciones actualizadas:", filteredSelections);
      ...
      //FUNCION  ANTERIOR
      //onSelectionChange(filteredSelections); // Llamar al padre para actualizar
      //return filteredSelections;
    //});

    // FUNCION INTERMEDIA
    // onSelectionChange(updatedSelections); // Llamar al padre para actualizar
    // return updatedSelections;
